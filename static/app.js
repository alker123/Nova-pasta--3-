...
window.login = async function() {
    ...
    const loginInput = emailField.value.trim();
    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            for (let id in users) {
                const u = users[id];
                if ((u.email === loginInput || u.username === loginInput) && u.password === password) {
                    userFound = u;
                    break;
                }
            }
            ...
            if (userFound) {
                ...
                window.location.href = userFound.rota; 
            } else {
                alert("Usuário ou senha incorretos.");
                passwordField.value = ""; // Limpa apenas a senha se errar
            }
        }
    } catch (error) {
